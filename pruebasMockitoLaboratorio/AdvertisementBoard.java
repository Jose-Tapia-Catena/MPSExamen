...
public class AdvertisementBoard {
     ...
     */
    public void publish(Advertisement advertisement, AdvertiserDatabase advertiserDatabase, PaymentDatabase paymentDatabase) {
        if (numberOfPublishedAdvertisements() == MAX_BOARD_SIZE) {
            throw new FullBoardException("Advertisement board is full");
        }
        if (advertisement.advertiser.equals("THE Company"))
            advertisementList.add(advertisement);
        else {
            if ((advertiserDatabase.findAdviser(advertisement.advertiser)) && (paymentDatabase.advertiserHasFunds(advertisement.advertiser)) && (!advertisementList.contains(advertisement))) {
                //contains use equals that is redefined by title and advertiser
                advertisementList.add(advertisement);
